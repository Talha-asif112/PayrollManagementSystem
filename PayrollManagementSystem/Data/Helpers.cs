...
public static class Helpers
{
    ...
    public static string GenerateJwt(this AppUser user, IConfiguration configuration)
    {
        ...
        var (_, credentials) = JwtSecurityKeyAndCredentials(configuration);
    ...
    /*    public static (string, long, string) HasAccess(this HttpContext context, string[] roles, string[] actions,
            params string[] resources)
        {
            var (role, userId) = context.GetRoleAndId();
            var message =
                $"User does not have Access to: {string.Join(", ", actions)} on Resource(s): {string.Join(", ", resources)}";
            if (string.IsNullOrEmpty(role) || userId == long.Empty)
            {
                return (role, userId, message);
            }
